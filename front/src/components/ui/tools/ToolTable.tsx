...
const ToolTable: React.FC<ToolTableProps> = ({
  ...
  const { selectedTools, toggleSelect, selectAll, clearSelection } =
    useToolStore();
  ...
  const allSelected =
    sorted.length > 0 && selectedTools.length === sorted.length;
  const partiallySelected =
    selectedTools.length > 0 && selectedTools.length < sorted.length;
      ...
      {selectedTools.length > 0 && (
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 bg-blue-50">
          ...
          <div className="flex items-center gap-2">
            ...
            <button className="px-3 py-2 rounded-lg bg-red-100 text-red-600 text-sm">
              Delete
            </button>
          </div>
        </div>
          ...
          <tbody>
            {sorted.map(tool => (
              ...
              >
                ...
                <td className="px-6 py-4">
                  {tool.status && (
                    <Badge status={normalizeStatus(tool.status)} />
                  )}
                </td>
